      ...
      const handleDeleteRecipe = async (recipeId) => {
        try {
          if (window.confirm("Are you sure you want to delete this recipe?")) {
            const response = await fetch(`http://localhost:2000/auth/recipe/${recipeId}`, {
                method: "DELETE",
              }
            );
          ...
          {recipes.length > 0 ? (
            recipes.map((recipe) => (
              <div key={recipe._id} className="Recipe">
                ...
                {recipe.cookingTime && (
                  <p className="cooking-time" style={{fontWeight: "bold"}}>Cooking Time: {recipe.cookingTime} minutes</p>
                )}
